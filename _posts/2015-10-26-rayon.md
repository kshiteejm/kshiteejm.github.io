...
- $$r_{j,0}$$ can be perpetuated to $$r_{j,t}\ \forall t$$. However intermediate data sources are dependent on the actual assignment of $$x_{j,t}$$ across timesteps. That is the non-zero dimensions in $$x_{j,t}$$ will be an indication of the data sources for $$x_{j,t+1}$$. This can be captured by changing the objective function to something as follows: $$maximize\ \frac{(r_{j,0} \cdot x_{j,0})}{c_{j,0}} + \sum_{j,t} \frac{(\delta (x_{j,t}) \cdot x_{j,t+1})}{c_{j,t}}$$. Here $$\delta (x_{j,t})$$ represents the signum function. However this changed objective function ceases to be a linear optimization problem. We can rather stick to the simplistic $$minimize\ \sum_{j,t} c_{j,t} * (r_{j,t} \cdot x_{j,t})$$ objective function with $$r_{j,0} = r_{j,t} \forall t$$. This is still effective in that it prevents deviations from the nodes on which initial data sources were and prevents allocations from spilling beyond this initial set for any timestep.