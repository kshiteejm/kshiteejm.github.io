...
- $$r_{j,0}$$ can be perpetuated to $$r_{j,t} \forall t$$. However intermediate data sources are dependent on the actual assignment $$x_{j,t}$$ is assigned. That is the non-zero dimensions in $$x_{j,t}$$ will be an indication of the data sources for $$x_{j,t+1}$$. This can be captured by changing the objective function to something as follows: $$minimize c_{j,0} * (r_{j,0} \cdot x_{j,0}) + \sum_{j,t} c_{j,t} * (\delta (x_{j,t}) \cdot x_{j,t+1})$$. However this ceases to be a linear optimization problem. $$\delta (x_{j,t})$$ represents the signum function.